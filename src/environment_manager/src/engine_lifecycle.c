 ...
 *
 * Handles creation, destruction, and validation of BatchEngine.
...
/* ============================================================================
 * Internal Helper: Compute per-sensor output size in floats
 * ============================================================================ */

static uint32_t sensor_config_output_floats(SensorType type, const SensorConfig* entry) {
    switch (type) {
        case SENSOR_TYPE_CAMERA_RGB:
            return (uint32_t)((size_t)entry->camera.width * entry->camera.height * 3);
        case SENSOR_TYPE_CAMERA_DEPTH:
        case SENSOR_TYPE_CAMERA_SEGMENTATION:
            return (uint32_t)((size_t)entry->camera.width * entry->camera.height);
        case SENSOR_TYPE_LIDAR_3D:
            return (uint32_t)((size_t)entry->lidar_3d.horizontal_rays *
                              entry->lidar_3d.vertical_layers);
        case SENSOR_TYPE_LIDAR_2D:
            return entry->lidar_2d.num_rays;
        case SENSOR_TYPE_IMU:       return 6;
        case SENSOR_TYPE_TOF:       return 1;
        case SENSOR_TYPE_POSITION:  return 3;
        case SENSOR_TYPE_VELOCITY:  return 6;
        case SENSOR_TYPE_NEIGHBOR:
            return (uint32_t)((size_t)entry->neighbor.k * 4);
        default:                    return 64;
    }
}
...
static ObjIOResult obj_to_world_gpu(Arena* arena, const char* path,
    ...
    *out_world = NULL;
    ...
    /* Collect surface brick list (allocate worst case, fill in one pass) */
        ...
        for (uint32_t by = 0; by < grid_y; by++) {
            for (uint32_t bx = 0; bx < grid_x; bx++) {
                ...
                if (classes->classes[idx] == BRICK_CLASS_SURFACE) {
                    surface_brick_list[surface_count * 3 + 0] = bx;
                    surface_brick_list[surface_count * 3 + 1] = by;
                    surface_brick_list[surface_count * 3 + 2] = bz;
                    surface_count++;
                }
            }
        }
     ...
     * Must run after GPU or CPU surface voxelization — the GPU shader doesn't
     * have a post-voxelization cleanup pass. */
...
/* ============================================================================
 * Internal Helper: Sync engine bounds from world
 * ============================================================================ */
...
static void create_default_sensors(SensorSystem* sensors, uint32_t total_agents) {
...
BatchEngine* engine_create(const EngineConfig* config, char* error_msg) {
    ...
    if (config->num_sensor_configs > 0 && config->sensor_configs != NULL) {
        uint32_t td = config->num_envs * config->agents_per_env;
        size_t sensor_mem = 0;
        for (uint32_t s = 0; s < config->num_sensor_configs; s++) {
            const SensorConfig* sc = &config->sensor_configs[s];
            size_t output_floats = sensor_config_output_floats(sc->type, sc);
            size_t pixels = 0;
            if (sc->type == SENSOR_TYPE_CAMERA_RGB ||
                sc->type == SENSOR_TYPE_CAMERA_DEPTH ||
                sc->type == SENSOR_TYPE_CAMERA_SEGMENTATION) {
    ...
    BatchEngine* engine = (BatchEngine*)arena_alloc_aligned(
        persistent, sizeof(BatchEngine), 64);
    if (engine == NULL) {
        arena_destroy(persistent);
        if (error_msg) snprintf(error_msg, ENGINE_ERROR_MSG_SIZE,
                               "Failed to allocate BatchEngine");
        return NULL;
    }

    /* Initialize engine structure */
    memset(engine, 0, sizeof(BatchEngine));
    engine->config = *config;
    engine->config.total_agents = config->num_envs * config->agents_per_env;
    ...
    /* 2. Platform state and parameter arrays (composite: rigid body + extensions) */
    {
        const PlatformVTable* vtable = config->platform_vtable;
        uint32_t state_ext = vtable ? vtable->state_extension_count : 0;
        uint32_t params_ext = vtable ? vtable->params_extension_count : 0;

        engine->states = platform_state_create(persistent, total_agents, state_ext);
        if (engine->states == NULL) {
            if (error_msg) snprintf(error_msg, ENGINE_ERROR_MSG_SIZE,
                                   "Failed to create platform state arrays");
            goto cleanup;
        }

        engine->params = platform_params_create(persistent, total_agents, params_ext);
        if (engine->params == NULL) {
            if (error_msg) snprintf(error_msg, ENGINE_ERROR_MSG_SIZE,
                                   "Failed to create platform params arrays");
            goto cleanup;
        }

        /* Initialize rigid body parameters with defaults */
        for (uint32_t i = 0; i < total_agents; i++) {
            rigid_body_params_init(&engine->params->rigid_body, i);
            /* Initialize platform-specific params via vtable */
            if (vtable && vtable->init_params) {
                vtable->init_params(engine->params->extension,
                                    engine->params->extension_count, i);
            }
            /* Initialize platform-specific state via vtable */
            if (vtable && vtable->init_state) {
                vtable->init_state(engine->states->extension,
                                   engine->states->extension_count, i);
            }
        }
    }
    ...
    if (config->obj_path != NULL) {
        ...
        vox_opts.use_custom_bounds = config->use_custom_bounds;
    ...
    engine->physics = physics_create(persistent, engine->frame_arena,
                                     &phys_config, total_agents,
                                     config->platform_vtable);
    ...
    /* 5. Collision system */
    engine->collision = collision_create(persistent, total_agents,
    ...
    if (config->num_sensor_configs > 0 && config->sensor_configs != NULL) {
        ...
        for (uint32_t s = 0; s < config->num_sensor_configs; s++) {
            const SensorConfig* sc = &config->sensor_configs[s];
            estimated += sensor_config_output_floats(sc->type, sc);
        }
        if (estimated > max_obs_dim) max_obs_dim = estimated;
    }
    engine->sensors = sensor_system_create(persistent, total_agents,
    ...
    if (config->num_sensor_configs > 0 && config->sensor_configs != NULL) {
        ...
        /* Use default sensors */
        create_default_sensors(engine->sensors, total_agents);
    }
    ...
    engine->gpu_sensor_ctx = gpu_sensor_context_create(total_agents);
    ...
    engine->episode_returns = (float*)arena_alloc_aligned(
        persistent, total_agents * sizeof(float), 32);
    if (engine->episode_returns == NULL) {
        if (error_msg) snprintf(error_msg, ENGINE_ERROR_MSG_SIZE,
                               "Failed to allocate episode_returns");
        goto cleanup;
    }
    memset(engine->episode_returns, 0, total_agents * sizeof(float));

    engine->episode_lengths = (uint32_t*)arena_alloc_aligned(
        persistent, total_agents * sizeof(uint32_t), 32);
    if (engine->episode_lengths == NULL) {
        if (error_msg) snprintf(error_msg, ENGINE_ERROR_MSG_SIZE,
                               "Failed to allocate episode_lengths");
        goto cleanup;
    }
    memset(engine->episode_lengths, 0, total_agents * sizeof(uint32_t));
    ...
    for (uint32_t env = 0; env < config->num_envs; env++) {
        for (uint32_t d = 0; d < config->agents_per_env; d++) {
            uint32_t idx = env * config->agents_per_env + d;
    ...
    engine->action_dim = config->platform_vtable ? config->platform_vtable->action_dim : 4;

    /* Observations buffer */
    size_t obs_size = total_agents * engine->obs_dim * sizeof(float);
    ...
    /* Actions buffer */
    size_t act_size = total_agents * engine->action_dim * sizeof(float);
    ...
    /* Rewards buffer */
    size_t rew_size = total_agents * sizeof(float);
    ...
    /* Dones buffer */
    size_t done_size = total_agents * sizeof(uint8_t);
    ...
    /* Detailed termination flag buffers */
    {
        uint8_t** term_ptrs[] = {
            &engine->term_success,
            &engine->term_collision,
            &engine->term_out_of_bounds,
            &engine->term_timeout
        };
        static const char* term_names[] = {
            "term_success", "term_collision",
            "term_out_of_bounds", "term_timeout"
        };
        for (uint32_t t = 0; t < 4; t++) {
            *term_ptrs[t] = (uint8_t*)arena_alloc_aligned(persistent, done_size, 32);
            if (*term_ptrs[t] == NULL) {
                if (error_msg) snprintf(error_msg, ENGINE_ERROR_MSG_SIZE,
                                       "Failed to allocate %s buffer", term_names[t]);
                goto cleanup;
            }
            memset(*term_ptrs[t], 0, done_size);
        }
    }

    /* --------------------------------------------------------------------------
     * Final initialization
     * (stats fields already zeroed by memset at engine allocation)
     * -------------------------------------------------------------------------- */
...
bool engine_is_valid(const BatchEngine* engine) {
    return engine != NULL
        ...
        && engine->rewards != NULL
        /* External buffers */
        && engine->observations != NULL
        && engine->actions != NULL
        && engine->rewards_buffer != NULL
        && engine->dones != NULL
        && engine->truncations != NULL
        /* Arenas */
        && engine->persistent_arena != NULL
        && engine->frame_arena != NULL;
}
...
size_t engine_memory_size(const EngineConfig* config) {
    ...
    uint32_t total_agents = config->num_envs * config->agents_per_env;

    size_t total = 0;

    /* BatchEngine struct */
    total += sizeof(BatchEngine);

    /* Drone state arrays (17 floats per drone) */
    total += total_agents * 17 * sizeof(float);

    /* Drone params arrays (15 floats per drone) */
    total += total_agents * 15 * sizeof(float);

    /* Episode tracking */
    total += total_agents * sizeof(float);    /* episode_returns */
    total += total_agents * sizeof(uint32_t); /* episode_lengths */
    total += total_agents * sizeof(uint32_t); /* env_ids */

    /* External buffers */
    uint32_t obs_dim = 15;  /* default for IMU + Position + Velocity */
    if (config->num_sensor_configs > 0 && config->sensor_configs != NULL) {
        uint32_t computed = 0;
        for (uint32_t s = 0; s < config->num_sensor_configs; s++) {
            computed += sensor_config_output_floats(
                config->sensor_configs[s].type, &config->sensor_configs[s]);
        }
        if (computed > 0) obs_dim = computed;
    }
    total += total_agents * obs_dim * sizeof(float);  /* observations */
    total += total_agents * 4 * sizeof(float);        /* actions */
    total += total_agents * sizeof(float);            /* rewards */
    total += total_agents * sizeof(uint8_t) * 2;      /* dones + truncations */
