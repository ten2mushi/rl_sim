 ...
 */
static BatchDroneEngine* create_engine_with_seed(uint64_t seed) {
 ...
 */
static BatchDroneEngine* create_large_engine(uint64_t seed, uint32_t num_envs, uint32_t drones_per_env) {
    EngineConfig cfg = engine_config_default();
...
TEST(same_seed_identical_positions) {
    ...
    BatchDroneEngine* e1 = create_engine_with_seed(SEED);
    BatchDroneEngine* e2 = create_engine_with_seed(SEED);
    ASSERT_NOT_NULL(e1);
    ASSERT_NOT_NULL(e2);
    ...
    for (uint32_t i = 0; i < total; i++) {
        ASSERT_EQ(e1->states->pos_x[i], e2->states->pos_x[i]);
        ASSERT_EQ(e1->states->pos_y[i], e2->states->pos_y[i]);
        ASSERT_EQ(e1->states->pos_z[i], e2->states->pos_z[i]);
    }
...
TEST(same_seed_identical_velocities) {
    ...
    uint32_t total = e1->config.total_drones;
    for (uint32_t i = 0; i < total; i++) {
        ASSERT_EQ(e1->states->vel_x[i], e2->states->vel_x[i]);
        ASSERT_EQ(e1->states->vel_y[i], e2->states->vel_y[i]);
        ASSERT_EQ(e1->states->vel_z[i], e2->states->vel_z[i]);
    }
...
TEST(same_seed_identical_quaternions) {
    ...
    uint32_t total = e1->config.total_drones;
    for (uint32_t i = 0; i < total; i++) {
        ASSERT_EQ(e1->states->quat_w[i], e2->states->quat_w[i]);
        ASSERT_EQ(e1->states->quat_x[i], e2->states->quat_x[i]);
        ASSERT_EQ(e1->states->quat_y[i], e2->states->quat_y[i]);
        ASSERT_EQ(e1->states->quat_z[i], e2->states->quat_z[i]);
    }
...
TEST(same_seed_identical_observations) {
    ...
    uint32_t total = e1->config.total_drones * engine_get_obs_dim(e1);
...
TEST(reset_and_rerun_determinism) {
    ...
    BatchDroneEngine* engine = create_engine_with_seed(SEED);
    ...
    memcpy(pos_x_1, engine->states->pos_x, total * sizeof(float));
    memcpy(pos_y_1, engine->states->pos_y, total * sizeof(float));
    memcpy(pos_z_1, engine->states->pos_z, total * sizeof(float));
    ...
    for (uint32_t i = 0; i < total; i++) {
        ASSERT_EQ(pos_x_1[i], engine->states->pos_x[i]);
        ASSERT_EQ(pos_y_1[i], engine->states->pos_y[i]);
        ASSERT_EQ(pos_z_1[i], engine->states->pos_z[i]);
    }
...
TEST(thread_count_independence) {
    ...
    for (int run = 0; run < 3; run++) {
        ...
        if (run == 0) {
            first_run_pos_x_0 = engine->states->pos_x[0];
            first_run_pos_y_0 = engine->states->pos_y[0];
            first_run_pos_z_0 = engine->states->pos_z[0];
        } else {
            ASSERT_EQ(first_run_pos_x_0, engine->states->pos_x[0]);
            ASSERT_EQ(first_run_pos_y_0, engine->states->pos_y[0]);
            ASSERT_EQ(first_run_pos_z_0, engine->states->pos_z[0]);
        }
...
TEST(rng_state_isolation) {
    ...
    float env0_drone0_x = engine->states->pos_x[0];
    float env1_drone0_x = engine->states->pos_x[drones_per_env];
...
TEST(floating_point_reproducibility) {
    ...
    for (int step = 0; step < STEPS; step++) {
        ...
        uint32_t total = e1->config.total_drones * ENGINE_ACTION_DIM;
        for (uint32_t i = 0; i < total; i++) {
...
TEST(nan_action_handling) {
    BatchDroneEngine* engine = create_engine_with_seed(12345);
    ASSERT_NOT_NULL(engine);
    ...
    uint32_t drone_count = engine->config.total_drones;
...
TEST(termination_collision) {
    ...
    engine->states->pos_x[0] = 1.0f;
    engine->states->pos_y[0] = 1.0f;
    engine->states->pos_z[0] = 1.0f;
...
TEST(termination_bounds) {
    ...
    engine->states->pos_x[0] = 10000.0f;
    engine->states->pos_y[0] = 10000.0f;
    engine->states->pos_z[0] = 10000.0f;
    ...
    float px = engine->states->pos_x[0];
    float py = engine->states->pos_y[0];
    float pz = engine->states->pos_z[0];
...
TEST(performance_1024_drones) {
    ...
    BatchDroneEngine* engine = create_large_engine(12345, NUM_ENVS, DRONES_PER_ENV);
    if (engine == NULL) {
    ...
    ASSERT_EQ(engine->config.total_drones, TARGET_DRONES);
